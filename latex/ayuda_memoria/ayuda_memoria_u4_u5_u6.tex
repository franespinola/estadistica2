...
\topic{violet}{Resumen: qué prueba usar}
\begin{center}
{\notesize
...
Prueba & Alt. a & Uso \\
\midrule
Signo & t-Uni. & 1 m. o pareada \\
U & t-Bi. & 2 m. indep. \\
H & ANOVA & $k{\geq}3$ m. indep. \\
Corridas & --- & Aleatoriedad \\
\bottomrule
\end{tabular}
}
\end{center}
Todas se usan cuando \textbf{no} se cumplen los supuestos paramétricos (normalidad) o los datos son ordinales/de rango.

\topic{violet}{Regla general de decisión}
\colorbox{gray!10}{\parbox{0.92\linewidth}{{\notesize
Si $p<\alpha$ (o $F_{calc}>F_{crit}$, o $|Z_{calc}|>Z_{crit}$) $\Rightarrow$ \textbf{Se rechaza $H_0$} (evidencia significativa a favor de $H_1$).\\
Si $p\geq\alpha$ (o $F_{calc}\leq F_{crit}$) $\Rightarrow$ \textbf{No se rechaza $H_0$} (datos insuficientes para descartarla).
}}}
...
\topic{orange}{Criterio para elegir el modelo}
1) Probar \textbf{recta} (mínimos cuadrados) y ver el diagrama de dispersión. 2) Si los puntos siguen una curva de forma conocida (exponencial, potencial, recíproca), aplicar \textbf{regresión curvilínea} (linealizar). 3) Si no se conoce la forma, usar \textbf{ajuste polinómico} probando grados sucesivos y elegir el grado con el \textbf{método de la varianza residual} (mayor caída de $\sigma_{res}^2$).
...
\topic{green}{Resumen: qué diseño usar}
\begin{itemize}[leftmargin=10pt,itemsep=1pt,topsep=1pt,parsep=0pt]
  \item \textbf{Completamente aleatorio:} mismo $n$ en cada tratamiento, sin factor de bloqueo.
  \item \textbf{Bloques aleatorios:} se controla 1 variable de bloqueo para reducir el error (ej. lavarropas).
  \item \textbf{Tamaños distintos ($n_i$):} igual al diseño 1 vía pero con muestras desbalanceadas.
\end{itemize}
En los 3 casos, si se rechaza $H_0$ se concluye que \textbf{al menos un tratamiento difiere} de los demás (no cuál en particular; para eso se usan pruebas de comparación múltiple, ej. Duncan).
