...
\textbf{Bondad de ajuste.} Compara la distribución observada con una distribución teórica propuesta: se determina si las diferencias son por azar o si el modelo teórico no ajusta.

\textbf{Poisson} modela la cantidad de sucesos en un intervalo fijo. \textbf{H0:} Los datos siguen la distribución de poisson. \textbf{H1:} No la siguen.
...
{\chico $\dashrightarrow$ hubieron 15 intervalos de 5 min en el q paso un solo auto. Se analizan 400 intervalos de 5 min (paso de autos) contra una Poisson con $\lambda$=4.6. En los extremos los valores son mas bajos: se agrupan las frec. esp. menores que 5 con el valor siguiente $\Rightarrow$ 10 grupos.\par}

Estadístico para ver si las diferencias entre lo observado y lo esperado son solo por casualidad:
\[
...
\]
{\chico Cada probabilidad teórica sale de la fórmula de Poisson con $\lambda = 4{,}6$ (ej.: $P(0)=e^{-4{,}6}=0{,}010$) y la frecuencia esperada de multiplicarla por los 400 intervalos ($400\cdot 0{,}010 = 4$; como es $<5$ se agrupa con la de un auto).\par}
\[
